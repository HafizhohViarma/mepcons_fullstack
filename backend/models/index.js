...
db.Users.hasMany(db.tb_Transaksi, {
  foreignKey: 'id_user',
  as: 'tb_transaksi',
});
db.tb_Transaksi.belongsTo(db.Users, {
  foreignKey: 'id_user',
  as: 'user',
});

// Relasi antara Video dan Transaksi
db.Video.hasMany(db.tb_Transaksi, {
  foreignKey: 'id_video',
  as: 'tb_transaksi',
});
db.tb_Transaksi.belongsTo(db.Video, {
  foreignKey: 'id_video',
  as: 'video',
});

// Relasi antara Ebook dan Transaksi
db.Ebook.hasMany(db.tb_Transaksi, {
  foreignKey: 'id_ebook',
  as: 'tb_transaksi',
});
db.tb_Transaksi.belongsTo(db.Ebook, {
  foreignKey: 'id_ebook',
  as: 'ebook',
});

// Relasi antara Kelas dan Transaksi
db.Kelas.hasMany(db.tb_Transaksi, {
  foreignKey: 'id_kelas',
  as: 'tb_transaksi',
});
db.tb_Transaksi.belongsTo(db.Kelas, {
  foreignKey: 'id_kelas',
  as: 'kelas',
});

// Relasi Video dan VideoFile
db.Video.hasMany(db.VideoFile, {
  foreignKey: 'id_video',
  as: 'file',
  onDelete: 'CASCADE',
});
db.VideoFile.belongsTo(db.Video, {
  foreignKey: 'id_video',
  as: 'video',
});

// Sinkronisasi database tanpa menghapus data yang ada
db.sequelize
  .sync({ force: false })
  .then(() => {
